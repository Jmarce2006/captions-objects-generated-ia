...
def search():
    ...
    if category == 'user':
        ...
        # 1️⃣ Get Photos from Description Search
        photo_results = Photo.query.whooshee_search(q).all()

        # 2️⃣ Get Photos from Tags (Using correct SQLAlchemy 2.0 query)
        tag_query = db.session.execute(select(Tag.id).where(Tag.name.ilike(f"%{q}%"))).scalars().all()

        if tag_query:
            tag_photo_ids = db.session.scalars(
                select(Photo.id).join(Photo.tags).where(Tag.id.in_(tag_query))
            ).all()
            tag_photo_results = Photo.query.filter(Photo.id.in_(tag_photo_ids)).all()
        else:
            tag_photo_results = []

        # 3️⃣ Merge & Remove Duplicates
        all_results = list({photo.id: photo for photo in (photo_results + tag_photo_results)}.values())

        # 4️⃣ Manual Pagination in Python
        ...
        class CustomPagination:
            """ A simple custom pagination object to mimic Flask-SQLAlchemy pagination. """
            ...
            def iter_pages(self, left_edge=2, left_current=2, right_current=2, right_edge=2):
                """ Mimics Flask-SQLAlchemy `iter_pages()` for generating page numbers. """
                last = 0
                for num in range(1, self.pages + 1):
                    if num <= left_edge or (num > self.page - left_current - 1 and num < self.page + right_current) or num > self.pages - right_edge:
                        if last + 1 != num:
                            yield None  # Ellipsis indicator
                        yield num
                        last = num

        pagination = CustomPagination(paginated_results, page, per_page, total_results)
